...
export default function AgentStatusBar({ logs, jobStatus }: AgentStatusBarProps) {
  ...
  const agents = [
    {
      ...
      desc: "Scans price metrics, sectors, and historical stock trends",
      status: getAgentStatus("MarketAgent"),
    },
    {
      id: "SentimentAgent",
      name: "Sentiment Analyst Agent",
      desc: "Evaluates social feeds and news channels via VADER",
      status: getAgentStatus("SentimentAgent"),
    },
    {
      id: "ReportAgent",
      name: "Risk & Report Writer Agent",
      desc: "Synthesizes multi-agent state into investment markdown",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-2xl overflow-hidden border border-slate-900 bg-[#090d16] text-[#c9d1d9] shadow-2xl relative font-mono text-[11px] leading-relaxed">
        {/* Terminal Header */}
        ...
        <div className="p-4 h-56 overflow-y-auto terminal-scroll space-y-2 flex flex-col">
          {logs.length === 0 ? (
            ...
            logs.map((l, idx) => {
              // Color-coding log outputs based on phase or agent
              ...
              const agentColor =
                l.agent === "MarketAgent"
                  ? "text-sky-500"
                  : l.agent === "SentimentAgent"
                  ? "text-emerald-500"
                  : "text-purple-500";

              return (
                <div key={idx} className="flex items-start gap-2 hover:bg-slate-900/40 py-0.5 rounded px-1 transition-colors">
                  <span className="text-slate-600 select-none">[{l.timestamp ? new Date(l.timestamp).toLocaleTimeString() : "--:--:--"}]</span>
