...
export default function PDFExportButton({ jobId }: PDFExportButtonProps) {
  ...
  const handlePdfDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Server returned status code ${response.status}. PDF might not be compiled yet.`);
      }
      ...
      link.setAttribute("download", `Portfolio_Analysis_Report_${jobId.slice(0, 8)}.pdf`);
  ...
  return (
    <div className="flex flex-col items-stretch gap-2 w-full">
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-[10px] p-2.5 rounded-lg flex items-center gap-1.5 font-bold uppercase tracking-wide">
